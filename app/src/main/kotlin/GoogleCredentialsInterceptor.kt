...
class GoogleCredentialsInterceptor( private val mCredentials: Credentials ) : ClientInterceptor {
    ...
    override fun <ReqT, RespT> interceptCall(
        ...
        val clientCall =
            object : ClientInterceptors.CheckedForwardingClientCall<ReqT, RespT>(newClientCallDelegate) { // io.grpc.ClientCall→ io.grpc.ForwardingClientCall→io.grpc.ClientInterceptorsCheckedForwardingClientCall
                //  ClientCall.start(ClientCall.listener,Metadata)はmisuse以外では例外を出すべきではない｡
                ...
                override fun checkedStart(responseListener: Listener<RespT>?, headers: Metadata)  {
                                                                // checkedStartのみが抽象メソッドなので実装する必要がある｡
                    ...
                    delegate().start(responseListener, headers) // 例外があるときは　delegate.start(ここのnewClientCallDelegate)を呼ぶ前にThrowする｡
                                                                // こうすると例外をresponseListenerに伝播できる｡
                }
            }
        return clientCall
    }
    ...
    @Throws(StatusException::class)
    fun removePort(uri: URI): URI { // 今回はUriにDefaultポートが設定されていたらPortを-1にする｡
        try {
            return URI( uri.scheme, uri.userInfo, uri.host, -1, uri.path, uri.query, uri.fragment )
        } catch (e: URISyntaxException) {
            throw Status.UNAUTHENTICATED
    ...
    fun toHeaders(metadata: Map<String, List<String>>): Metadata { // Map<String,List>をMetadataクラスに変換する｡
